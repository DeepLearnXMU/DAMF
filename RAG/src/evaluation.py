...
def ufs(prediction, ground_truths, lang='zh'):
    if lang == 'en':
        prediction = word_tokenize(prediction)
        return max([unigram_f1_score(prediction, word_tokenize(gt)) for gt in ground_truths])
    elif lang == 'zh':
        return max([unigram_f1_score(prediction, gt) for gt in ground_truths])
